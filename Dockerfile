...
RUN wget https://www.python.org/ftp/python/3.9.12/Python-3.9.12.tgz && \
    ...
    make install && \
    cd .. && \
    rm -vr Python-3.9.12 Python-3.9.12.tgz 

# RUN python3 -m pip --upgrade pip && \
#     python3 -m install pysam

RUN pip3 install --upgrade pip && \
    pip3 install pysam

# Install Bowtie2
RUN wget https://excellmedia.dl.sourceforge.net/project/bowtie-bio/bowtie2/2.3.4.1/bowtie2-2.3.4.1-linux-x86_64.zip --no-check-certificate && \
...
# Install Tophat2
run wget http://ccb.jhu.edu/software/tophat/downloads/tophat-2.1.1.Linux_x86_64.tar.gz --no-check-certificate && \
...
RUN wget https://excellmedia.dl.sourceforge.net/project/samtools/samtools/0.1.18/samtools-0.1.18.tar.bz2 --no-check-certificate && \
...
RUN wget https://cloud.r-project.org/src/base/R-3/R-3.5.0.tar.gz --no-check-certificate && \
...
# Install Bedtools
RUN wget https://github.com/arq5x/bedtools2/releases/download/v2.25.0/bedtools-2.25.0.tar.gz --no-check-certificate && \
...
# Install Cufflinks
RUN wget http://cole-trapnell-lab.github.io/cufflinks/assets/downloads/cufflinks-2.2.1.Linux_x86_64.tar.gz --no-check-certificate && \
...
# Get LIONS
RUN wget https://github.com/ababaian/LIONS/archive/master.zip --no-check-certificate && \
    unzip master.zip && \
    rm -v master.zip && \
